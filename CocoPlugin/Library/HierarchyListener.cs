...
//NOTE: This class is from the VS SDK 2008
namespace at.jku.ssw.Coco.VSPlugin.Library {

    #region HierarchyEventArgs
    ...
    internal class HierarchyListener : IVsHierarchyEvents, IDisposable {
        ...
        public int OnItemAdded(uint itemidParent, uint itemidSiblingPrev, uint itemidAdded) {
            // Check if the item is a grammar file.
            Debug.WriteLine("\n\tOnItemAdded\n");
            string name;
            if (!IsGrammarFile(itemidAdded, out name)) {
                return VSConstants.S_OK;
            }

            // This item is a grammar file, so we can notify that it is added to the hierarchy.
            if (null != onItemAdded) {
        ...
        public int OnItemDeleted(uint itemid) {
            Debug.WriteLine("\n\tOnItemDeleted\n");
            // Notify that the item is deleted only if it is a grammar file.
            string name;
            if (!IsGrammarFile(itemid, out name)) {
                return VSConstants.S_OK;
            }
        ...
        private bool IsGrammarFile(uint itemId, out string canonicalName) {
            ...
            return (0 == string.Compare(extension, CocoLanguageService.EXTENSION, StringComparison.OrdinalIgnoreCase));
        }

        /// <summary>
        /// Do a recursive walk on the hierarchy to find all the grammar files in it.
        ...
        private void InternalScanHierarchy(uint itemId) {
            ...
            while (VSConstants.VSITEMID_NIL != currentItem) {
                // If this item is a grammar file, then send the add item event.
                string itemName;
                if ((null != onItemAdded) && IsGrammarFile(currentItem, out itemName)) {
