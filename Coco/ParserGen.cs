...
public class ParserGen {
    ...
    Writer gen;       // Coco-Plugin: generated parser source file
	StringWriter err; // generated parser error messages
	ArrayList symSet = new ArrayList();
    BufferHelper bufferHelper;	   // Coco-Plugin
    Mapping map = new Mapping();  // Coco-Plugin: maps atg file position to parser file position
		...
		if (pos != null) {
			...
			while (buffer.Pos <= pos.end) {
				while (ch == CR || ch == LF) {  // eol is either CR or CRLF or LF
                    // Coco-Plugin begin
                    bufferHelper.PositionToLine(buffer.Pos, out tline, out tcolumn);
                    map.Set(gen.Line, gen.Coloumn, tline, tcolumn, 1);
                    // Coco-Plugin end
	...
	public void WriteParser () {
        ...
        gen = new Writer(g.OpenGen("Parser.cs")); // Coco-Plugin: new Writer
		...
		gen.Close();
        map.Write(Path.Combine(tab.srcDir, "Parser.atgmap"), tab.srcName); // Coco-Plugin
		buffer.Pos = oldPos;
	}
